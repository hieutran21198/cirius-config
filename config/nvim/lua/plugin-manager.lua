if not vim.loop.fs_stat(_G.lazy_path) then
	vim.fn.system({
		"git",
		"clone",
		"--filter=blob:none",
...
vim.opt.rtp:prepend(_G.lazy_path)
...
M.setup = function(input)
	...
	local p = input.path or vim.fn.stdpath("data") .. "/lazy/lazy.nvim"
	...
	lazy.setup("plugins", vim.tbl_deep_extend("force", lazy_opts, input.lazy_opts or {}))
end
